...
def make_choice():
    ...
    while choice not in valid_choices:
        ...
        choice = input(
            "Make your selection using the option number. Select 9 to list all options: "
        )
...
def get_task_from_user(msg="Input the id of the task you would like to work with: "):
    ...
    while not task:
        ...
        if count >= help_count:
            print(
                "You seem to be having trouble selecting a task.  Please choose from the following list of tasks."
            )
            print_all_tasks()
...
def change_task_complete_status(status):
    ...
    task = get_task_from_user(
        f"Input the id of the task you would like to mark {status_text}: "
    )
    if task:
        if status:
